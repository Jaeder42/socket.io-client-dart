...
class Manager extends EventEmitter {
  ...
  this.reconnectionAttempts = options['reconnectionAttempts'] ?? double.INFINITY;
  ...
  onopen([_]) {
     ...
     */
    onpong([_]) {
      this.emitAll('pong', new DateTime.now().millisecondsSinceEpoch - this.lastPing);
    }
    ...
    packet(Map packet) {
      ...
      var self = this;
      if (packet.containsKey('query') && packet['type'] == 0) packet['nsp'] += '''?${packet['query']}''';
...
class _Backoff {
  ...
  _Backoff({min = 100, max = 10000, jitter = 0, factor = 2}):
    _ms = min, _max = max, _factor = factor {
